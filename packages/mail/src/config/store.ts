...
export async function saveSession(
  ...
  await saveProductSession("mail", session, username);
}
...
export async function clearSession(): Promise<void> {
  ...
  await clearProductSession("mail");
}

/** Clear Mail product-local + shared session (dual-mint rollback). */
export async function clearMailState(): Promise<void> {
  await clearSession();
}
