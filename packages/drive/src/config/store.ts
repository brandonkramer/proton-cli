...
export async function saveSession(
  ...
  await saveProductSession("drive", session, username);
}
...
export async function loadSession(
  ...
  const shared = await loadProductSession("drive", expectedUsername);
...
export async function clearSession(): Promise<void> {
  ...
  await clearProductSession("drive");
}

/** Clear Drive product-local + shared session (dual-mint rollback). */
export async function clearDriveState(): Promise<void> {
  await clearSession();
}
