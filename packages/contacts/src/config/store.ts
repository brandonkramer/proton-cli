...
export async function saveSession(
  ...
  await saveProductSession("contacts", session, username);
}
...
export async function loadSession(
  ...
  const shared = await loadProductSession("contacts", expectedUsername);
...
export async function clearSession(): Promise<void> {
  ...
  await clearProductSession("contacts");
}

/** Clear Drive product-local + shared session (dual-mint rollback). */
export async function clearContactsState(): Promise<void> {
  await clearSession();
}
