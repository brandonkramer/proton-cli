...
export interface UnlockedCalendarKeys {
...
async function fetchUser(session: Session, fetchImpl?: typeof fetch) {
  const data = await calendarApi<{ User: { ID: string; Name: string; Keys: { ID: string; Version: number; PrivateKey: string; Token?: string | null; Primary?: number; Active?: number }[] } }>(
    USERS_PATH,
    { session, fetchImpl },
  );
  return data.User;
...
async function fetchKeySalts(session: Session, fetchImpl?: typeof fetch) {
  const data = await calendarApi<{ KeySalts: { ID: string; KeySalt: string | null }[] }>(
    KEY_SALTS_PATH,
    { session, fetchImpl },
  );
...
async function fetchAddresses(session: Session, fetchImpl?: typeof fetch) {
  const data = await calendarApi<{ Addresses: ProtonAddress[] }>(
    ADDRESSES_PATH,
    { session, fetchImpl },
  );
...
/** Unlock user + address keys via shared core CryptoProxy (REQ-CAL-003). */
export async function unlockCalendarKeys(
  options: UnlockFetchOptions,
): Promise<UnlockedCalendarKeys> {
