...
export async function saveSession(
  ...
  await saveProductSession("calendar", session, username);
}
...
export async function loadSession(
  ...
  const shared = await loadProductSession("calendar", expectedUsername);
...
export async function clearSession(): Promise<void> {
  ...
  await clearProductSession("calendar");
}

/** Clear Calendar product-local + shared session (dual-mint rollback). */
export async function clearCalendarState(): Promise<void> {
  await clearSession();
}
